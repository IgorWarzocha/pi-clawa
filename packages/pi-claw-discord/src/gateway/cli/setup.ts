...
export async function runSetup(args: string[]): Promise<void> {
  ...
  clack.intro('pi-claw-discord setup');
    ...
    const result = await clack.select({
      ...
      options: [
        ...
        },
        {
          ...
          label: 'allowlist',
          hint: 'Only respond in manually registered channels (pi-claw-discord register ...)',
        },
      ],
  ...
  if (interactive && isLinux()) {
    ...
    if (installDaemon) {
      ...
      try {
        ...
        clack.log.success('pi-claw-discord-gateway.service is active');
      } catch (err) {
        s.stop('Service installation failed.');
        clack.log.error(errorMessage(err));
        clack.log.info('You can install manually later: pi-claw-discord daemon install && pi-claw-discord daemon start');
      }
    }
  }
...
export function buildConfigFile(options: {
  ...
  return [
    '# Generated by: pi-claw-discord setup',
    '# Or edit manually. See: pi-claw-discord help',
    '',

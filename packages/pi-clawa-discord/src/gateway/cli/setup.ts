...
const DEFAULT_DATA_DIR = resolve(DEFAULT_WORKING_DIR, '.pi/clawa-discord');
...
export async function runSetup(args: string[]): Promise<void> {
  ...
  clack.intro('pi-clawa-discord setup');
    ...
    const result = await clack.select({
      ...
      options: [
        ...
        },
        {
          ...
          label: 'allowlist',
          hint: 'Only respond in manually registered channels (pi-clawa-discord register ...)',
        },
      ],
  ...
  if (interactive && isLinux()) {
    ...
    if (installDaemon) {
      ...
      try {
        ...
        clack.log.success('pi-clawa-discord-gateway.service is active');
      } catch (err) {
        s.stop('Service installation failed.');
        clack.log.error(errorMessage(err));
        clack.log.info('You can install manually later: pi-clawa-discord daemon install && pi-clawa-discord daemon start');
      }
    }
  }
...
export function buildConfigFile(options: {
  ...
  return [
    '# Generated by: pi-clawa-discord setup',
    '# Or edit manually. See: pi-clawa-discord help',
    '',

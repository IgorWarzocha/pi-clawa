...
- Use only the route tags shown in the current Discord turn or route config. Do not invent channels like `[#general]`.
- Use one or more final routing blocks:
  - `[#configured-channel]: public room note`
...
- Use `message_discord` only for explicit sends/reactions outside final routing blocks. It requires `channel`: `dm` or an exact routed `#channel` name.

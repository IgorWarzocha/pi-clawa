...
import type { ClawasConfig, WorkerDefinition, WorkerPromptProfile } from './types.js'
...
function getPromptTargetPath(projectRoot: string, profile: ResolvedWorkerPromptProfile): string {
  return path.join(projectRoot, '.pi', 'system-prompts', `clawas-${profile}.md`)
}
...
export async function syncClawasSystemPromptLinks(
  projectRoot: string,
  config: ClawasConfig,

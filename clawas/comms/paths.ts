...
const DEFAULT_CONTROL_SOCKET_DIR = 'clawas-control'
...
export function getSocketPath(sessionId: string): string {
  return path.join(getClawasControlDir(), `${sessionId}${SOCKET_SUFFIX}`)
}

function getAliasPath(alias: string): string {
  return path.join(getClawasControlDir(), `${alias}${ALIAS_SUFFIX}`)
}

export async function ensureControlDir(): Promise<void> {
  await fs.mkdir(getClawasControlDir(), { recursive: true })
}
...
export async function removeAliasesForSocket(socketPath: string | null): Promise<void> {
  ...
  const controlDir = getClawasControlDir()
  try {

...
test('pulse root AGENTS is skipped while pulse-specific AGENTS loads', async () => {
  ...
  try {
    ...
    await writeFile(join(root, 'pulses', 'hey-clawa', 'AGENTS.md'), 'HEY CLAWA')
    await writeFile(join(root, 'pulses', 'hey-clawa', 'PULSE.md'), '# Hey\n')
    ...
    const result = await handlers.get('tool_result')?.(
      {
        toolName: 'read',
        isError: false,
        input: { path: join(root, 'pulses', 'hey-clawa', 'PULSE.md') },

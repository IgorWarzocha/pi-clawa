...
import { HowabandaRpcWorker } from './rpc-worker.js'
...
import type { HowabandaConfig, HowabandaState, WorkerDefinition } from './types.js'
import { HowabandaWorkerEventRouter } from './worker-event-router.js'
...
function resetStateForRestart(state: HowabandaState, timestamp: number): void {
  state.events = []
  state.nextEventId = 1
 ...
 */
export class HowabandaDaemon {
  private readonly state: HowabandaState
  private readonly workers = new Map<string, HowabandaRpcWorker>()
  ...
  private readonly eventRouter: HowabandaWorkerEventRouter
  private started = false
  private stopping = false

  constructor(
    projectRoot: string,
    private readonly config: HowabandaConfig,
    ...
    this.controlPlaneRoot = join(
      projectRoot,
      '.pi',
      this.burrowDefaults?.controlPlaneDir ?? 'howabanda',
    )

    this.eventRouter = new HowabandaWorkerEventRouter({
      state: this.state,
  ...
  async ensureWorkerRunning(workerId: string): Promise<void> {
    ...
    if (workerState.manualSession) {
      throw new Error(
        `Worker ${workerId} is in a manual session and is disconnected from ${this.getBandaName()}`,
      )
    }
  ...
  async markWorkerDetached(workerId: string, label = 'manual session'): Promise<void> {
    // Detached/manual means HOWABANDA deliberately lets go of orchestration.
    // The worker stays reachable to the human, but not to banda tools.
  ...
  private async startWorker(workerId: string): Promise<void> {
    ...
    try {
      await worker.start()
      await worker.setSessionName(getWorkerSessionName(definition, this.burrowDefaults))
  ...
  private attachWorkerListeners(workerId: string, worker: HowabandaRpcWorker): void {
  ...
  private async stopAll(): Promise<void> {
    // Shutdown should be best-effort across the whole banda. One unhappy worker
    ...
    try {
      ...
      for (const [index, result] of results.entries()) {
        ...
        pushEvent(
          this.state,
          worker?.definition.id ?? this.getBandaId(),
  ...
  private getBandaName(): string {
    return this.burrowDefaults?.bandaName ?? 'HOWABANDA'
  }

  private getMainClawName(): string {
    return this.burrowDefaults?.mainClawName ?? 'Howaclawa'
  }

  private getBandaId(): string {
    return this.getBandaName()
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
  }

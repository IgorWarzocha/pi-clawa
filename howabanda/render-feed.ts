...
import { clampMonitorWorkerIndex, type HowabandaMonitorState } from './monitor-state.js'
...
export function renderMonitorLines(
  ...
  const header = `${theme.fg('accent', '╭─')} ${theme.bold(theme.fg('accent', burrowDefaults.bandaName))} ${theme.fg('dim', `${liveWorkers}/${workers.length} live${busyLabel}${activeLabel}`)}`

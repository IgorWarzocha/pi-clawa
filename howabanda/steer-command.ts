...
function resolveSteerTarget(runtime: HowabandaRuntime, args: string): SteerTarget | string {
  ...
  if (!activeWorker) return 'No HOWABANDA workers are available to steer.'
  return { worker: activeWorker, message: trimmed }
}

function resolveJumpTarget(runtime: HowabandaRuntime, args: string): WorkerState | string {
  const trimmed = args.trim()
  if (!trimmed) {
    const activeWorker = runtime.getActiveMonitorWorker()
    return activeWorker ?? 'No HOWABANDA workers are available to jump into.'
  }
...
export function registerSteerCommand(pi: ExtensionAPI, runtime: HowabandaRuntime): void {
  pi.registerCommand('steer', {
    description:
      'Send a private steer note to the active HOWABANDA claw, or to /steer <slot> <message>',
...
export function registerJumpCommand(pi: ExtensionAPI, runtime: HowabandaRuntime): void {
  pi.registerCommand('jump', {
    description: 'Open the active HOWABANDA claw in a manual panel, or /jump <slot|worker>',
    handler: async (args, ctx) => {
      ...
      if (!runtime.canOpenManualPanel()) {
        ctx.ui.notify(
          `${runtime.getBurrowDefaults().bandaName} manual takeover requires Herdr or tmux`,
          'warning',
        )
        return
      }
...
export function registerHowabandaMonitorShortcuts(
  pi: ExtensionAPI,
  runtime: HowabandaRuntime,
): void {
  pi.registerShortcut('alt+w', {
    description: 'Fold or open the HOWABANDA monitor widget',
    handler: async () => {
  ...
  pi.registerShortcut('alt+q', {
    description: 'Select previous HOWABANDA claw in the monitor widget',
    handler: async () => {
  ...
  pi.registerShortcut('alt+e', {
    description: 'Select next HOWABANDA claw in the monitor widget',
    handler: async () => {
      runtime.selectRelativeMonitorWorker(1)
    },
  })

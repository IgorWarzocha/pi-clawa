...
import { HOWABANDA_MAIL_MESSAGE_TYPE } from './outbound.ts'
import {
  extractBandaReportText,
  normalizeDiscordReplyText,
  shouldAutoRelayFinalAssistantToDiscord,
  shouldReportHowabandaFinalToMain,
...
test('extractBandaReportText keeps explicit banda content only', () => {
  assert.equal(extractBandaReportText('[BANDA]\nhello from worker'), 'hello from worker')
  assert.equal(extractBandaReportText('[BANDA] hello from worker'), 'hello from worker')
  assert.equal(extractBandaReportText('plain assistant text'), null)
})
...
test('getLastMailMessageTimestamp returns nearest HOWABANDA mail timestamp', () => {
  assert.equal(

...
import { sendHowabandaSessionMessage } from './client.js'
import {
...
export async function reportFinalAssistantMessageToMain(
  ...
  const workerId = options.workerId ?? process.env.PI_HOWABANDA_WORKER_ID ?? 'unknown'
  ...
  const reportMode = process.env.PI_HOWABANDA_REPORT_MODE?.trim() || 'auto'
  if (reportMode === 'off') {
    return
  }
  ...
  const reportContent =
    reportMode === 'explicit' ? extractBandaReportText(message.content) : message.content
  if (!reportContent) {
    return
  }
  ...
  await sendHowabandaSessionMessage(options.targetSessionId, {

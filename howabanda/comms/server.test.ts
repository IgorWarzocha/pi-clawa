...
import {
  ...
  HowabandaCommsServer,
  shouldDeliverHowabandaMailAsUserMessage,
} from './server.ts'
...
test('only discord-gateway mail uses the worker-facing Discord user-message envelope', () => {
  ...
  assert.equal(shouldDeliverHowabandaMailAsUserMessage(details), true)
...
test('worker reports are not routed through the chunky user-message envelope', () => {
  ...
  assert.equal(shouldDeliverHowabandaMailAsUserMessage(details), false)
  const message = buildWorkerUserMessage('Got the update.', details)

  assert.match(message, /^\[HOWABANDA worker update\]/)
...
function makeServerHarness() {
  ...
  const server = new HowabandaCommsServer(pi as never, () => 'worker')
...
test('for_context startup mail is stored as custom context without triggering a worker turn', () => {
  ...
  ;(
    ...
    messageType: 'session',
    mode: 'steer',
    sender: { workerId: 'main-claw', workerTitle: 'Howaclawa' },
    kind: 'instruction',
    intent: 'for_context',
    visibility: 'worker',
  })
  ...
  assert.equal((calls[0].args[0] as { customType?: string }).customType, 'howabanda-session')
})

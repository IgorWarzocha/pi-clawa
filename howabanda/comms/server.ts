...
import { HOWABANDA_MAIL_MESSAGE_TYPE } from './outbound.js'
import {
  ensureControlDir,
  getSocketPath,
...
const IS_MANUAL_SESSION = process.env.PI_HOWABANDA_MANUAL_SESSION === '1'
const LEGACY_SESSION_MESSAGE_TYPE = 'howabanda-session'
const LEGACY_REPORT_MESSAGE_TYPE = 'howabanda-report'
...
function resolveMessageKind(command: HowabandaSendCommand): HowabandaMessageKind {
  if (command.kind) {
    return command.kind
  }
...
export function buildMessageDetails(
  sender: HowabandaSenderInfo | undefined,
  discordContext: HowabandaDiscordContext | undefined,
  kind: HowabandaMessageKind,
...
export function buildWorkerUserMessage(
  ...
  return [
    '[HOWABANDA worker update]',
    `Source: ${title}`,
    `Kind: ${details.kind}; intent: ${details.intent}; visibility: ${details.visibility}`,
    'This is a HOWABANDA coordination message, not direct speech from Igor unless the content says so.',
    'Current trigger:',
    message,
  ].join('\n')
}

export function shouldDeliverHowabandaMailAsUserMessage(
  details: ReturnType<typeof buildMessageDetails>,
): boolean {
  return details.workerId === 'discord-gateway'
}

function getLegacyMailCustomType(command: HowabandaSendCommand): string {
  return command.messageType === 'report' ? LEGACY_REPORT_MESSAGE_TYPE : LEGACY_SESSION_MESSAGE_TYPE
}

function shouldTriggerTurn(command: HowabandaSendCommand): boolean {
...
function parseCommand(line: string): {
  ...
  try {
    const parsed = JSON.parse(line) as HowabandaCommsCommand
...
function writeResponse(socket: Socket, response: HowabandaRpcResponse): void {
  try {
...
/**
 * Minimal embedded session-control server for HOWABANDA.
 * It only supports fire-and-forget sends and reading the last assistant message.
 */
export class HowabandaCommsServer {
  ...
  private async createServer(): Promise<Server> {
    if (!this.socketPath) {
      throw new Error('HOWABANDA comms socket path is not ready')
    }
  ...
  private handleCommand(command: HowabandaCommsCommand, socket: Socket): void {
  ...
  private handleSendCommand(ctx: ExtensionContext, command: HowabandaSendCommand): void {
    ...
    const customType = HOWABANDA_MAIL_MESSAGE_TYPE
    ...
    if (shouldDeliverHowabandaMailAsUserMessage(details)) {

...
import { HOWABANDA_DELIVERY_MESSAGE_TYPE, HOWABANDA_MAIL_MESSAGE_TYPE } from './outbound.js'
import type { HowabandaExtractedDelivery, HowabandaExtractedMessage } from './types.js'
...
function getHowabandaMailDetails(entry: Record<string, unknown>): Record<string, unknown> | null {
  const customType = entry.customType
  if (
    customType !== HOWABANDA_MAIL_MESSAGE_TYPE &&
    customType !== 'howabanda-session' &&
    customType !== 'howabanda-report'
  ) {
    return null
  }
...
export function getLastMailMessageDetails(
  ...
  for (let index = branch.length - 1; index >= 0; index -= 1) {
    ...
    const details = getHowabandaMailDetails(entry)
    if (!details) {
      continue
    }
...
export function getLastAssistantMessage(
  ctx: ExtensionContext,
): HowabandaExtractedMessage | undefined {
  const branch = ctx.sessionManager.getBranch()
...
export function getLastDeliveryMessage(
  ctx: ExtensionContext,
): HowabandaExtractedDelivery | undefined {
  const branch = ctx.sessionManager.getBranch()
  ...
  for (let index = branch.length - 1; index >= 0; index -= 1) {
    ...
    if (entry.customType !== HOWABANDA_DELIVERY_MESSAGE_TYPE) {
      continue
    }
...
export function getLastMailMessageTimestamp(ctx: ExtensionContext): number | undefined {
  ...
  for (let index = branch.length - 1; index >= 0; index -= 1) {
    const entry = branch[index] as Record<string, unknown>
    if (!getHowabandaMailDetails(entry)) {
      continue
    }
...
export function getLastDiscordSourceMessageId(ctx: ExtensionContext): string | undefined {
  ...
  for (let index = branch.length - 1; index >= 0; index -= 1) {
    ...
    // Reply anchoring is only safe for the current HOWABANDA mail turn.

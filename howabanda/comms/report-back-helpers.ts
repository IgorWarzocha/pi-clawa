...
export function extractBandaReportText(content: string): string | null {
  const trimmed = content.trim()
  if (!trimmed) {
    return null
  }

  const withStandaloneDirective = trimmed.match(/^\[BANDA\]\s*(?:\n+([\s\S]*))?$/i)
  if (withStandaloneDirective) {
    return (withStandaloneDirective[1] ?? '').trim()
  }

  const withInlineDirective = trimmed.match(/^\[BANDA\]\s+([\s\S]+)$/i)
...
export function shouldSkipAutoMainClawStatusRelay(options: {
  ...
  // HOWABANDA mail arrived, the final assistant text is only local closure.
...
export function shouldReportHowabandaFinalToMain(options: {

import { HOWABANDA_EVENT_LIMIT } from './config.js'
import type { FeedEvent, HowabandaState, WorkerDefinition, WorkerState } from './types.js'
...
export function getWorkerState(state: HowabandaState, workerId: string): WorkerState {
  const worker = state.workers.find((entry) => entry.definition.id === workerId)
  if (!worker) {
    throw new Error(`Unknown HOWABANDA worker: ${workerId}`)
  }
  return worker
}
...
export function pushEvent(
  ...
  state.events = [...state.events, event].slice(-HOWABANDA_EVENT_LIMIT)
  state.nextEventId += 1
}

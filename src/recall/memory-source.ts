...
import { compareResults, includesAllTags, normalizeTags, scoreText } from './scoring.js'
...
export function searchMemory(
  ...
  try {
    const rows = (
      tokens.length === 0 && requiredTags.length === 0
        ? db.prepare('SELECT id, ts, text, tags FROM memories ORDER BY ts DESC LIMIT ?').all(limit)
        : db.prepare('SELECT id, ts, text, tags FROM memories ORDER BY ts DESC').all()
    ) as MemoryRow[]
    return rows

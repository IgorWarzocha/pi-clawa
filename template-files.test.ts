...
test('template helpers detect existing files and copy templates assertively', async () => {
  ...
  try {
    ...
    await writeFile(join(source, 'CLAW.md'), 'template claw', 'utf8')
    await mkdir(target, { recursive: true })
    await writeFile(join(target, 'CLAW.md'), 'existing claw', 'utf8')

    assert.deepEqual(await findExistingTemplateFiles(source, target), ['CLAW.md'])
    assert.equal(await hasAllTemplateFiles(source, target), false)

    const result = await copyTemplateFiles(source, target)

    assert.deepEqual(result.copied.sort(), ['AGENTS.md', 'CLAW.md'].sort())
    assert.equal(await readFile(join(target, 'AGENTS.md'), 'utf8'), 'template agents')
    assert.equal(await readFile(join(target, 'CLAW.md'), 'utf8'), 'template claw')
